...
public class ChatActivity extends AppCompatActivity implements EventListener, FileItemClickedAction {
    ...
    // Online chat polling (Supabase REST)
    private final Handler onlinePollHandler = new Handler();
    private final HashSet<String> seenOnlineMessageIds = new HashSet<>();
    private boolean onlinePollingStarted = false;
...
//        event_radio_disconnected.addListener(this);
        ...
        // Start polling for online messages (Supabase)
        startOnlinePolling();
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ChatMessage temp = mssg.isEncrypted ? EncryptionController.getInstance().getDecryptedMessage(mssg) :
                        mssg;
                // Track ids to avoid duplicate rendering from online polling
                if (mssg.mssgId != null) seenOnlineMessageIds.add(mssg.mssgId);
