...
public class LoginActivity extends AppCompatActivity {
    ...
    private void checkFirebase(final String userID, final String password){
        Log.e(TAG, SUB_TAG+"Checking firebase (Supabase)");

        // Network login is asynchronous; run it off the UI thread and only then
        // transition to the next screen.
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                final LoggedInUser remoteUser = FirebaseController.checkLogInInfoSync(userID, password, LoginActivity.this);
    ...
    private void success(){
        ...
        // Ensure the user can "get back" immediately after logging in.
        // Avoid depending on DB write timing (remote login writes can be async).
        if (MainActivity.nearbyConnectionHandler == null)
            MainActivity.nearbyConnectionHandler = new com.sjsu.boreas.OfflineConnectionHandlers.NearbyConnectionHandler(this);

        android.content.Intent intent = new android.content.Intent(this, LandingPage.class);
