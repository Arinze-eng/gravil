...
public class SupabaseRestApi {
    ...
    public JSONArray searchProfiles(String accessToken, String q) throws IOException {
        // Search by code exact (if 4 digits) OR name ilike
        String or;
    ...
    public void updateLastSeen(String accessToken, String userId) throws IOException {
        JSONObject body = new JSONObject();
        body.put("last_seen", "now()");

        // PostgREST doesn't accept now() as string. Use prefer: return=minimal and call via RPC? easiest: set to current timestamp in client.
        // We'll overwrite with client-side ISO timestamp.
    }
    ...
    public JSONObject getOrCreateConversation(String accessToken, String me, String other) throws IOException {
        String url = SupabaseConfig.SUPABASE_URL + "/rest/v1/conversations?select=id,user_a,user_b&or=(and(user_a.eq." + enc(me) + ",user_b.eq." + enc(other) + "),and(user_a.eq." + enc(other) + ",user_b.eq." + enc(me) + "))&limit=1";
    ...
    public JSONArray fetchMessages(String accessToken, String conversationId, String sinceIso) throws IOException {
        String url = SupabaseConfig.SUPABASE_URL + "/rest/v1/messages?select=id,conversation_id,sender_id,receiver_id,kind,body,media_url,media_type,media_size_bytes,created_at,read_at&conversation_id=eq." + enc(conversationId) + "&order=created_at.asc";
        if (sinceIso != null) {
    ...
    public void markConversationRead(String accessToken, String conversationId, String me) throws IOException {
        // Mark all unread messages where I'm receiver
        JSONObject body = new JSONObject();
