...
public class FirebaseController {
    ...
    /**
     * Register user synchronously. Returns HTTP status code.
     *
     * - 201/200: success
     * - 409: uid collision (already exists)
     */
    public static int registerUserSync(final LoggedInUser user, final Context context) {
        try {
            ...
            body.put("public_key", user.getPublicKey());
            body.put("password", user.getPassword());
            body.put("private_key", user.getPrivateKey());
            ...
            Log.e(TAG, SUB_TAG + "registerUserSync error", e);
            return -1;
        }
    }
    ...
    /**
     * Synchronous login (must be called off the UI thread).
     *
     * Reason: the old async implementation returned before the network request
     * finished, so LoginActivity would treat valid credentials as a failure.
     */
    public static LoggedInUser checkLogInInfoSync(final String userID, final String password, final Context context) {
        Log.e(TAG, SUB_TAG + "Checking the provided user ID and password on Supabase (sync)");
        ...
        try {
            ...
            // Save locally (async). LoginActivity no longer depends on this completing
            // before navigation.
            localDatabaseReference.registerUser(liu);

            Log.e(TAG, SUB_TAG + "Login success for " + liu.getUid());
            return liu;
    ...
    // Legacy signature kept for backward compatibility (async).
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    try {
                        ...
                        for (int i = 0; i < arr.length(); i++) {
                            ...
                            // Reconstruct enough fields for UI + local DB
                            String senderUid = row.optString("sender_uid", "");
                            String receiverUid = row.optString("receiver_uid", "");
                            boolean isMy = MainActivity.currentUser.getUid().equals(senderUid);

                            User sender = isMy ? MainActivity.currentUser : new User(senderUid, senderUid, 0.0, 0.0, "");
                            User recipient = isMy ? new User(receiverUid, receiverUid, 0.0, 0.0, "") : MainActivity.currentUser;
                            ...
                            try {
                                ...
                                if (!createdAt.isEmpty()) {
                                    // keep it simple; ordering is server-side anyway
                            ...
                            ChatMessage cm = new ChatMessage(
                                    sender,
                                    recipient,
                                    row.optString("mssg_id", ""),
                                    row.optString("mssg_text", ""),
                                    t,
                                    isMy,
