...
type (
	...
	KnownPeer struct {
		...
		// ReportedUsingPeerID is the last exit-node (proxy) selection
		// reported by this peer over the status protocol.
		ReportedUsingPeerID string `json:"reportedUsingPeerID,omitempty"`
	}
	BlockedPeer struct {
		// Hex-encoded multihash representing a peer ID
		PeerID      string `json:"peerId"`
